...
def get_server_from_token(token):
    ...
    try:
        ...
        lock_region = decoded.get("lock_region", "VN")
...
def adding_friend_custom():
    ...
    friend_uid = request.args.get('id')
    server_name = request.args.get('region', 'VN')
...
# Run Server
# ----------------------------
# ... (toàn bộ code hiện tại của bạn giữ nguyên đến hết)
...
def status_reporter():
    while True:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Server đang chạy ổn định... API sẵn sàng!")
        time.sleep(8)  # In ra mỗi 8 giây

# Khởi động thread báo trạng thái ngay khi script bắt đầu chạy
...
if __name__ == '__main__':
    ...
    print(f"\n=== FreeFire Friend API Server ===")
    print(f"Đang khởi động trên port {port}...")
    print(f"Truy cập: http://127.0.1.1:{port} hoặc từ mạng LAN")
    print(f"Endpoints: /add | /remove | /info | /token | /health")
